...
public class Cache {
    ...
    private int map(int blockNO) {
        ...
        byte[] addrTag = calculateTag(blockNO);   // 获得内存地址blockNO所对应的tag
        int start = setNO * setSize;
        int end = (setNO + 1) * setSize - 1;
        for (int i = start; i <= end; i++) {
            if (cacheInstance.isMatch(i, addrTag.toString().toCharArray())) {   // 命中该行
     ...
     */
    private byte[] calculateTag(int blockNO) {
        ...
        int diff = 26 - tagStr.length();
        for (int i = 0; i < diff; i++) {
            tagStr.append("0");
        }
        return tagStr.toString().getBytes();
    }
    ...
    private int loadBlock(int blockNO) {
        ...
        byte[] addrTag = calculateTag(blockNO);
        if (SETS == 32) {
            cacheInstance.update(setNO, addrTag.toString().toCharArray(), Memory.getMemory().read(Transformer.intToBinary(String.valueOf(Cache.LINE_SIZE_B * blockNO)), Cache.LINE_SIZE_B));
            return setNO;
        }
        return this.replacementStrategy.replace(setNO * setSize, (setNO + 1) * setSize - 1, addrTag.toString().toCharArray(), Memory.getMemory().read(Transformer.intToBinary(String.valueOf(Cache.LINE_SIZE_B * blockNO)), Cache.LINE_SIZE_B));
    }

...
public class NBCDU {
    ...
    DataType add(DataType src, DataType dest) {
        ...
        CF = "0";
        OF = "0";
        return new DataType(addAddition(src.toString(), dest.toString()));
    }
    ...
    DataType sub(DataType src, DataType dest) {
        ...
        CF = "0";
        OF = "0";
        return new DataType(subAddition(src.toString(), dest.toString()));
    }

    public String addAddition(String a,String b){
        String result;

        if (isPositive(a) && !isPositive(b)) {
            result = subAddition("1100".concat(b.substring(4)), a);
        } else if (!isPositive(a) && isPositive(b)) {
            result = subAddition("1100".concat(a.substring(4)), b);
        } else {
            String sign = "";
            if (isPositive(a) && isPositive(b)) {
                sign = "1100";
    ...
    public String subAddition(String a,String b){
        String result;

        if (!isPositive(a) && isPositive(b)) {
            result = addAddition("1100".concat(a.substring(4)), b);
        } else if (isPositive(a) && !isPositive(b)) {
            result = addAddition("1101".concat(a.substring(4)), b);
        } else if (!isPositive(a) && !isPositive(b)) {
            result = subAddition("1100".concat(b.substring(4)), "1100".concat(a.substring(4)));
        } else {
            result = addAddition("1100".concat(getInversion(a.substring(4))), b);
            if (OF.equals("0")) {
